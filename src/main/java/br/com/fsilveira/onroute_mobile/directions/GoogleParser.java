...
public class GoogleParser extends XMLParser implements Parser {
	...
	public GoogleParser(String feedUrl) throws DirectionsException {
		super(feedUrl);
	}
	...
	public Route parse() throws DirectionsException {
		...
		try {
			...
			for (int index = 0; index < legs.length(); index++) {
				...
				final JSONObject startLocation = leg.getJSONObject("start_location");
				route.addMarker(new LatLng(startLocation.getDouble("lat"), startLocation.getDouble("lng")));

				if (index + 1 == legs.length()) {
					final JSONObject endLocation = leg.getJSONObject("end_location");
					route.addMarker(new LatLng(endLocation.getDouble("lat"), endLocation.getDouble("lng")));
				}
				...
				route.plusLength(leg.getJSONObject("distance").getInt("value"));
